...
const Pokedex = () => {
    ...
    const pagesNumbers = [];
    for (
        let i = firstPage ? firstPage : 1;
        i <= lastPage && i <= totalPages;
        i++
    ) {
        pagesNumbers.push(i);
    }
    ...
    useEffect(() => {
        axios
            .get(`https://pokeapi.co/api/v2/pokemon/?limit=1126&offset=0/`)
            .then((res) => setPokemons(res.data.results));

        axios
            .get(`https://pokeapi.co/api/v2/type/?offset=0&limit=1126/`)
            .then((res) => {
            ...
            <div className="container-pages">
                <button
                    ...
                    className={`button-pages ${
                        currentRow <= 1 ? 'none' : 'block'
                    } bg-purple font-button-pages`}
                >
                    {html.menor}
                </button>
                {pagesNumbers.map((page) => (
                    <button
                        ...
                        key={page}
                        className={`button-pages ${
                            selectPage === page ? 'bg-green' : 'bg-purple'
                        } border`}
                    >
                ...
                <button
                    ...
                    className={`button-pages ${
                        currentRow >= totalRows ? 'none' : 'block'
                    } bg-purple font-button-pages`}
                >
                    {html.mayor}
                </button>
